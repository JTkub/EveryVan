...
ALTER TABLE passenger ADD COLUMN IF NOT EXISTS passport_no VARCHAR(30);
CREATE UNIQUE INDEX IF NOT EXISTS uq_passenger_id_card ON passenger(id_card) WHERE id_card IS NOT NULL AND id_card <> '';
CREATE UNIQUE INDEX IF NOT EXISTS uq_passenger_passport_no ON passenger(passport_no) WHERE passport_no IS NOT NULL AND passport_no <> '';
...
CREATE UNIQUE INDEX uq_active_trip_seat ON booking(trip_id,seat_number) WHERE booking_status NOT IN ('cancelled','completed','alighted');
...
CREATE TABLE IF NOT EXISTS travel_follower (follower_id BIGSERIAL PRIMARY KEY, passenger_user_id INT NOT NULL REFERENCES app_user(user_id) ON DELETE CASCADE, follower_user_id INT NOT NULL REFERENCES app_user(user_id) ON DELETE CASCADE, relationship VARCHAR(10) NOT NULL CHECK(char_length(relationship) BETWEEN 1 AND 10), follower_status VARCHAR(10) NOT NULL DEFAULT 'pending' CHECK(follower_status IN ('pending','accepted','denied')), created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), UNIQUE(passenger_user_id,follower_user_id));
